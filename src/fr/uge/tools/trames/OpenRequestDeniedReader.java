...
public final class OpenRequestDeniedReader implements Reader<OpenRequestDeniedReader.OpenRequestDenied>, Trame {
  
  
    public record OpenRequestDenied(FILE file, int chunkNumber)  {
    ...
    private OpenRequestDenied value;
    ...
    public ProcessStatus process(ByteBuffer buffer) {
        ...
        state = State.DONE;
        value = new OpenRequestDenied(file, chunkNumber);
        return ProcessStatus.DONE;
    }

    @Override
    public OpenRequestDenied get() {
        if (state != State.DONE) {
            throw new IllegalStateException();
        }
        return value;
