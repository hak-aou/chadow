...
public final class HidenRequestReader implements Reader<HidenRequestReader.HidenRequest>, Trame {
  
  
    public record HidenRequest(FILE file, int chunkNumber, int token)  {
    ...
    public ProcessStatus process(ByteBuffer buffer) {
        ...
        state = State.DONE;
        value = new HidenRequest(file, chunkNumber, token);
        return ProcessStatus.DONE;
    }

    @Override
    public HidenRequest get() {
        if (state != State.DONE) {
            throw new IllegalStateException();
        }
        return value;
